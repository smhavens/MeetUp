...
-- -----------------------------------------------------
-- procedure list_all_pending_invites_for_user
-- -----------------------------------------------------

DELIMITER $$
USE `meetup`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `list_all_pending_invites_for_user`(IN userID VARCHAR(8))
BEGIN
	SELECT * FROM invitedto i 
	WHERE i.userID=userID AND i.rsvp is null;
END$$
