...
DELIMITER $$
USE `meetup`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `invite_guest`(IN guestFirstName VARCHAR(15), IN guestLastName VARCHAR(15), IN eventID VARCHAR(8))
BEGIN
	DECLARE userName VARCHAR(8);
    
	SELECT userID INTO userName
    FROM user
    WHERE firstName=GuestFirstName AND lastName=GuestLastName;
